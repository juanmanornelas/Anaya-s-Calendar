...
// ── Photo Storage ─────────────────────────────────────────────────────────────
const PHOTOS_FILE = path.join(DATA_DIR, 'anaya-photos.json');
function readPhotos() {
  if (!fs.existsSync(PHOTOS_FILE)) return {};
  try { return JSON.parse(fs.readFileSync(PHOTOS_FILE, 'utf8')); }
  catch(e) { return {}; }
}
function writePhotos(store) {
  fs.writeFileSync(PHOTOS_FILE, JSON.stringify(store), 'utf8');
}

// POST /photos/upload  { jobId, jobName, uploadedBy, photos: [{data, name, type}] }
app.use(express.json({ limit: '50mb' })); // photos need more space
app.post('/photos/upload', (req, res) => {
  try {
    const { jobId, jobName, uploadedBy, photos } = req.body;
    if (!jobId || !photos || !photos.length) {
      return res.status(400).json({ error: 'Missing jobId or photos' });
    }
    const store = readPhotos();
    if (!store[jobId]) store[jobId] = { jobName: jobName||'', photos: [] };
    const ts = new Date().toISOString();
    const added = photos.map(p => ({
      id: Date.now() + '_' + Math.random().toString(36).slice(2,7),
      data: p.data,       // base64 data URL
      name: p.name || 'photo.jpg',
      type: p.type || 'image/jpeg',
      uploadedBy: uploadedBy || 'Worker',
      uploadedAt: ts,
    }));
    store[jobId].photos = [...store[jobId].photos, ...added];
    // Keep max 50 photos per job to avoid bloat
    store[jobId].photos = store[jobId].photos.slice(-50);
    writePhotos(store);
    console.log(`[photos] +${added.length} for job ${jobId} by ${uploadedBy}`);
...
// GET /photos/:jobId
app.get('/photos/:jobId', (req, res) => {
  try {
    const store = readPhotos();
    const entry = store[req.params.jobId];
    res.json({ ok: true, photos: entry ? entry.photos : [], jobName: entry ? entry.jobName : '' });
  } catch(err) {
    res.status(500).json({ error: err.message });
  }
});

// DELETE /photos/:jobId/:photoId  (admin only — no auth but obscure)
